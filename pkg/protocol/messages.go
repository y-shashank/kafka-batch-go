...
type JobMessage struct {
	...
	// BatchCounted means this job already emitted an executed/failed touch event
	// (first finish). Success may still emit later for on_success.
...
type EventMessage struct {
	...
	JobID        string `json:"job_id"`
	Status       string `json:"status"` // success | failed | executed
	WorkerClass  string `json:"worker_class"`
...
// CallbackMessage is produced when a batch finalizes or reaches Sidekiq-style complete.
type CallbackMessage struct {
	BatchID        string                 `json:"batch_id"`
	Outcome        string                 `json:"outcome"` // success | success_only | complete
	TotalJobs      int64                  `json:"total_jobs"`
	CompletedCount int64                  `json:"completed_count"`
	FailedCount    int64                  `json:"failed_count"`
	TouchedCount   int64                  `json:"touched_count,omitempty"`
	...
	// Preclaimed is true when the ledger Lua already HSETNX'd callback claim fields.
